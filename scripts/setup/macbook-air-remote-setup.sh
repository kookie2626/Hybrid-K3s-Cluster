...
# Hybrid K3s Cluster using static IP + router port forwarding (no VPN).
#
# What this script does:
#   1. Installs Homebrew (if missing)
#   2. Installs required CLI tools: kubectl, helm (optional)
...
#      server address to use the public IP
#
# Usage:
...
#   MASTER_PUBLIC_IP    Public IP (or LAN IP) of the N100 master node
#   MASTER_SSH_PORT     External SSH port forwarded to the master (default: 22)
...
echo "--------------------------------------------------------------------"
echo
info "To copy the key to a server (on the local LAN):"
info "  ssh-copy-id -i ${SSH_KEY}.pub <user>@<server-lan-ip>"
echo

# --------------------------------------------------------------------------- #
# 4. Collect master node details
# --------------------------------------------------------------------------- #
if [[ -z "${MASTER_PUBLIC_IP:-}" ]]; then
    read -r -p "Enter the public IP (or LAN IP) of the N100 master node: " MASTER_PUBLIC_IP
fi
if [[ -z "${MASTER_SSH_PORT:-}" ]]; then
    read -r -p "Enter the SSH port for the master node (router-forwarded port, e.g. 2210; leave blank for 22): " MASTER_SSH_PORT
...
[[ -n "${MASTER_PUBLIC_IP}" ]] || die "MASTER_PUBLIC_IP must not be empty."
[[ -n "${MASTER_SSH_PORT}" ]] || die "MASTER_SSH_PORT must not be empty."
...
# --------------------------------------------------------------------------- #
# 6. Fetch kubeconfig from master and patch the server address
# --------------------------------------------------------------------------- #
...
info "Fetching kubeconfig from ${MASTER_USER}@${MASTER_PUBLIC_IP} (port ${MASTER_SSH_PORT})..."
...
REMOTE_KUBECONFIG=$(ssh \
    ...
    warn "  ssh -p ${MASTER_SSH_PORT} ${MASTER_USER}@${MASTER_PUBLIC_IP} 'sudo cat /etc/rancher/k3s/k3s.yaml' > ${KUBECONFIG_FILE}"
    warn "  sed -i '' 's|https://127.0.0.1:6443|https://${MASTER_PUBLIC_IP}:6443|g' ${KUBECONFIG_FILE}"
    warn "  chmod 600 ${KUBECONFIG_FILE}"
    REMOTE_KUBECONFIG=""
}

if [[ -n "${REMOTE_KUBECONFIG}" ]]; then
    # Replace the loopback address with the public IP of the master
    PATCHED=$(echo "${REMOTE_KUBECONFIG}" \
        | sed "s|https://127.0.0.1:6443|https://${MASTER_PUBLIC_IP}:6443|g" \
        | sed "s|https://localhost:6443|https://${MASTER_PUBLIC_IP}:6443|g")
    ...
    ok "kubeconfig saved to ${KUBECONFIG_FILE} (server: https://${MASTER_PUBLIC_IP}:6443)"
fi
...
if [[ -f "${KUBECONFIG_FILE}" ]]; then
    ...
    if kubectl get nodes --request-timeout=10s 2>/dev/null; then
        ...
        warn "Make sure port 6443 is forwarded on your router and the master node is running."
    fi
fi
...
ok "       HostName ${MASTER_PUBLIC_IP}   # same public IP, different port"
ok "       Port <worker-ssh-port>         # e.g. 2220"
ok "       User <username>"
